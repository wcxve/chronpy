...
from .dwt import get_scaling, get_wavelet
...
def get_Wjm1n(h, g, j, n, Wjn1, Wjn2):
    ...
    for t in range(0, N):
        for lvl in range(0, L):
            index = int((t + (2 ** (j - 1)) * lvl) % N)
            Wjm1n[t] = Wjm1n[t] + an[lvl] * Wjn1[index] + bn[lvl] * Wjn2[index]
    return Wjm1n
...
def get_MODWPT(X, name, J):
    ...
    """
    assert isinstance(J, int), 'Level of MODWPT must be an integer'
    assert J >= 1, 'Level of MODWPT must be higher or equal to 1'
    g = get_scaling(name)
    h = get_wavelet(g)
    W = [[X]]
...
def inv_MODWPT(WJ, name, J):
    ...
    assert J >= 1, 'Level of MODWPT must be higher or equal to 1'
    assert len(WJ) == 2**J, (
        f'MODWPT decomposition at level {J} must contain {int(2**J)} vectors'
    )
    g = get_scaling(name)
    h = get_wavelet(g)
    Wj = WJ
