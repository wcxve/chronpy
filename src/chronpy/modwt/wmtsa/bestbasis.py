...
from .dwpt import compute_basisvector, compute_c, get_DWPT
...
def compute_m(W, typecost, normX=1, delta=0.5, p=1):
    ...
    if typecost == 'entropy':
        ...
        assert delta > 0, 'The threshold must be strictly positive'
        if abs(W) > delta:
            m = 1.0
        else:
            m = 0.0
    elif typecost == 'lp':
        assert (p > 0) and (p < 2), 'We must have 0 < p < 2 for the lp norm'
        m = (abs(W)) ** p
    else:
...
def bestbasis(W, X, typecost, delta=0.5, p=1):
    ...
    for j in range(J, 0, -1):
        for n in range(0, int(2 ** (j - 1))):
            if cost[j - 1][n] <= cost[j][int(2 * n)] + cost[j][int(2 * n + 1)]:
                base[j - 1][n] = 1
                base[j][int(2 * n)] = 0
                base[j][int(2 * n + 1)] = 0
            else:
    ...
    if base[0][0] == 1:
        ...
        for j in range(1, len(base)):
            for n in range(0, int(2**j)):
                if base[j][n] == 1:
                    weights = W[j][
                        int(n * N / (2**j)) : int((n + 1) * N / (2**j))
                    ]
                    cjn = c[j - 1][n]
                    vectors = compute_basisvector(cjn, name, N)
                    for lvl in range(0, len(weights)):
                        w.append(weights[lvl])
                    for lvl in range(0, np.shape(vectors)[1]):
                        b.append(vectors[:, lvl])
    return (w, b)
